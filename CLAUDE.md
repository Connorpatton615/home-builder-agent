...
- **Heartbeat watchdog** (`watchers/watchdog.py`) — runs every 10 min via launchd. Reads `.heartbeats/*.json` (each launchd job beats on success), alerts on staleness via macOS notification + `.heartbeat_alerts.log`. Per-job thresholds: dashboard/reconcile 300s, inbox 1500s, morning-brief 25h, client-update 8d. Plist: `~/Library/LaunchAgents/com.chadhomes.watchdog.plist`.

All five home-builder launchd jobs (dashboard-watcher, inbox-watcher, reconcile, morning-brief, client-update) plus `hb-bridge` now emit structured JSON to stderr via `home_builder_agent.observability.json_log.configure_json_logging()`. Schema: `ts | level | service | event | correlation_id | message | payload | traceback?`. Live JSON at `/tmp/<job>.stderr.log`; legacy print() text continues to `/tmp/<job>.stdout.log`. The configure helper has a TTY gate so interactive Terminal runs (`hb-reconcile --dry-run`, etc.) stay human-readable on the existing print() statements alone.
...
18. **Learning loop — active personalization + product instinct** — four-loop system on top of migration 004's `user_signal` + `user_profile` foundation. (1) **Active capture** of Chad's decisions with rationale via 5 new signal types (decision_rationale, correction_received, preference_stated, pattern_confirmed/rejected, clarification_asked/answered) — open-enum, no migration. (2) **Clarification protocol** — when `hb-chad` asks Chad for missing info (4 legitimate cases, 5 hard blocks, daily cap of 2, topic memory prevents re-asks, per-channel format). (3) **Weekly reflection ritual** — top-3 learned patterns surface in morning brief + Cmd+/ pop-over for confirm/reject/suppress, prevents silent profile drift. (4) **Product instinct** — new `hb-instinct` agent runs Sundays, watches Chad for friction patterns (manual repetition, repeated questions, consistent overrides, copy-paste workflows, long unstructured asks) and emails Connor evidence-backed *build candidates* — Chad never has to file feature requests, the system files them on his behalf. Spec: [`docs/specs/learning-loop.md`](docs/specs/learning-loop.md). Phase 1 (Chad-facing, Parts A-D) ~9 days; Phase 2 (`hb-instinct`, Part E) ~3.5 days, gated on 30+ days of signal history. Depends on migration 004 cut + `hb-profile` Phase A.2.
...
- **Brand palette is the source of truth for every renderer.** `brand/logo.svg` defines three colors — `#080b0f` ink (background), `#22c55e` signal (single accent), `#edf2f7` bone (foreground). Both renderers (Mac desktop + iOS mobile) inherit this palette and the discipline that comes with it: **color is reserved for status semantics; decorative color is forbidden.** Status hue lives only in the green/amber/red urgency band and the muted-grey done state. Mobile diverges from desktop only on layout density and input affordances (sun-readable type, glove-tolerant tap targets, thumb-arc bottom-weighted layout, haptics replacing hover) — never on brand. Specs: [`docs/specs/desktop-design-language.md`](docs/specs/desktop-design-language.md) + [`docs/specs/mobile-design-language.md`](docs/specs/mobile-design-language.md).
- **The agent files Chad's feature requests on his behalf — Chad doesn't ask Connor for features.** Per [`docs/specs/learning-loop.md`](docs/specs/learning-loop.md) Part E: `hb-instinct` runs weekly, watches Chad for friction patterns (manual repetition, repeated questions, consistent overrides, copy-paste workflows, long unstructured asks), and emails *Connor* (not Chad) evidence-backed build candidates with specific signal-ID citations. Chad never sees this loop; he just keeps working. The roadmap is built on observed friction, not assumptions. This is the difference between a vendor and a partner — and the most valuable artifact this whole system produces for the company, separate from its value for Chad.
