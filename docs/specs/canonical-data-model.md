...
**Clarification — dispatcher state writes are not renderer writes.** The Notification dispatcher (entity 13) is engine-owned at the rule level even when its implementation is deployed in a different process. The Patton AI iOS Shell hosts the APNs leg of the dispatcher — device tokens, bundle IDs, and APNs credentials live shell-side, so the push-sending code lives there too. When the dispatcher writes Notification delivery-side state (`delivered_at`, `viewed_at`, `push_id`), it is acting as the engine, not as a renderer. These writes are an authorized exception to Rule 1, scoped to the specific delivery-side fields and only when performed by the dispatcher process (Supabase service role, RLS-bypass). User-driven Notification fields (`dismissed_at`) and all Event lifecycle fields (`acknowledged_at`, `resolved_at`, `acknowledgement_actor`) still flow through UserAction → engine. The dispatcher is deployment-flexible; the rule it honors is unchanged. Cross-reference: `~/Projects/patton-ai-ios/docs/03_build/turtle_contract_v1.md` § 1.3 Push notifications.
